...
describe('Documentation Deployment Pipeline Integration Tests', () => {
  ...
  test('VitePress configuration is valid', () => {
    const configFiles = [
      join(DOCS_DIR, '.vitepress', 'config.ts'),
      join(DOCS_DIR, '.vitepress', 'theme', 'index.ts')
    ]
  ...
  test('build output structure is correct', () => {
    ...
    expect(indexContent).toMatch(/<meta name="description" content="Professional cross-platform UI component library">/)
  ...
  test('search functionality is built correctly', () => {
    ...
    const componentRecords = searchIndex.records.filter((record: any) =>
      record.url.includes('/components/')
    )
  ...
  test('static assets are optimized', () => {
    ...
    // Count static assets
    const assetFiles = execSync(`find "${assetsDir}" -type f`, { encoding: 'utf8' })
      .trim()
      .split('\n')
      .filter(Boolean)
  ...
  test('performance requirements are met', () => {
    ...
    const totalSize = execSync(`du -sb "${BUILD_DIR}"`, { encoding: 'utf8' })
      .split('\t')[0]

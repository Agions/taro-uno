...
export const VirtualList = React.forwardRef<HTMLElement, VirtualListProps<unknown>>((props, ref) => {
  const {
    data,
    renderItem,
    itemHeight,
    ...
    for (let i = start; i <= end; i++) {
      ...
      const key =
        typeof itemKey === 'function'
          ? itemKey(position.data, i)
          : ((position.data as Record<string, unknown>)[itemKey as string] as string);

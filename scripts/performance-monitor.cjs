...
class PerformanceMonitor {
  ...
  async collectPerformanceMetrics() {
    try {
      ...
      const perfScripts = Object.keys(packageJson.scripts).filter(
        (script) => script.includes('perf') || script.includes('performance'),
      );

      const perfDeps = Object.keys(packageJson.devDependencies || {}).filter(
        (dep) => dep.includes('performance') || dep.includes('perf') || dep.includes('vite-plugin'),
      );
